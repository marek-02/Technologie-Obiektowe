...
public class PhotoDownloader {
    ...
    public Observable<Photo> searchForPhotos(List<String> searchQueries) {
        List<Observable<Photo>> photos = new ArrayList<>();

        for(String searchQuery : searchQueries) {
            photos.add(searchForPhotos(searchQuery).subscribeOn(Schedulers.io()));
        }

        return Observable.merge(photos);
//        return Observable.just(searchQueries).flatMap(x -> searchForPhotos(x)); zamiast całej reszty
    }
