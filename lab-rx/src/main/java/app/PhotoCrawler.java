...
public class PhotoCrawler {
    ...
    public void downloadPhotosForQuery(String query) {
        photoDownloader.searchForPhotos(query).subscribe(next -> photoSerializer.savePhoto(next),
                error -> log.log(Level.SEVERE, "Searching photos error", error));
    }

    public void downloadPhotosForMultipleQueries(List<String> queries) {
        photoDownloader.searchForPhotos(queries).subscribe(next -> photoSerializer.savePhoto(next),
                error -> log.log(Level.SEVERE, "Searching photos error", error));
    }
}
